...
class Input
{
	...
	public:
		...
		int GetBind( std::string name ) const;
		bool BindExists( std::string name ) const;
		bool IsBound( int key ) const;
		bool OnKey( SDL_Scancode key ) const;
		bool OnKey( std::string name ) const;
		bool OnKeyDown( SDL_Scancode key ) const;
		bool OnKeyDown( std::string name ) const;
		bool OnKeyUp( SDL_Scancode key ) const;
		bool OnKeyUp( std::string name ) const;
