...
class Importer
{
	public:
		...
		bool ImportImage( const char* filename, bool vertical_flip = true, FilterType minFilter = FilterType::LINEAR, FilterType magFilter = FilterType::LINEAR, WrapType wrapping = WrapType::REPEAT ) const;
	...
	static void SetTextureToMaterial( Material* material, std::string name );
		ImageData* ReImportImage( const char* filename, bool vertical_flip = true  ) const;
	protected:
		ImporterImp* imp_;
};
